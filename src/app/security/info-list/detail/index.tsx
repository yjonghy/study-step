...
import XSSCSRF from "./XSSCSRF"
import CSP from "./CSP"
import Authentication from "./Authentication"

export default function SecurityDetailList(props: any) {
    return (
        <article className="w-full flex flex-col justify-center items-center">
            ...
            {props.data === "CORS"                                && <CORS />}
            {props.data === "XSS & CSRF"                         && <XSSCSRF />}
            {props.data === "CSP (Content Security Policy)"      && <CSP />}
            {props.data === "인증 전략 (Session / JWT / OAuth)"   && <Authentication />}
        </article>
    )
}
