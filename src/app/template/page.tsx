        ...
        <section className="w-full h-full">
            <article className="py-[40px] rounded-[8px] px-[30px] bg-white/70 shadow-shadow15">
                ...
                <p className="mt-[8px] body-sm text-gray080">
                    analytics firebase console 모니터링하며 서비스를 검토하합니다
            ...
            <article className="mt-[20px] py-[40px] rounded-[8px] px-[30px] bg-white/70 shadow-shadow15">
                    ...
                    <div className="flex flex-col gap-[2px]">
                        <div className="flex flex-col gap-[4px]">
                            ...
                            <div className="w-fit p-[8px] flex flex-col body-xs text-white bg-blue070/50 rounded-[8px] shadow-shadow16_15">
                                <p>장소데이터 업로드과정에서 업로드 완료시, display : none 으로 되어있는 img 태그 src에 웹 서비스 안에서 쓰는 이미지 크기를 사이즈별로 쿼리 요청하여 이미지 미리 생성</p>
                                ...
                                <p>img 브라우저에 렌더링 될때에도 loading 있음을 설명하고 onload 활용, boolean 설정하여 loader ui 도입</p>
                            </div>
                        </div>

                        <div className="mt-[12px] flex flex-col gap-[4px]">
                            <div className="flex items-center gap-[4px]">
                                <p className="heading-xs text-gray060">▶ 서비스 제공지역이 미국으로, 타임존 이슈 발생</p>
                            </div>
                            <p className="w-fit p-[8px] body-xs text-white bg-blue070/50 rounded-[8px] shadow-shadow16_15">Intl api를 사용하여 브라우저에 저장된 timeZone과 en-us 를 지역값으로 가지는 option 객체로 받아온 데이터의 시간을 바꾼뒤 string으로 변환하여 표시</p>
                        </div>

                        <div className="mt-[12px] flex flex-col gap-[4px]">
                            <div className="flex items-center gap-[4px]">
                                <p className="heading-xs text-gray060">▶ 모달창이 열린 상태에서 브라우저 뒤로가기 클릭할때 페이지 상태 유지 후 모달창이 닫혀야 하지만 페이지가 뒤로가는 이슈 발생</p>
                            </div>
                            <div className="w-fit p-[8px] flex flex-col body-xs text-white bg-blue070/50 rounded-[8px] shadow-shadow16_15">
                                <p>모달창 열때 기존 주소창에 해쉬(#)추가하여 페이지 이동은 없이 스택추가</p>
                                <p>스켈레톤 ui, 로더 ui 부재 속에서 api 요청 과정(요청, 대기, 완료)을 qa, pm 등 비 개발자에게 설명하며, 도입 필요성 피력 후 도입</p>
                                <p>window.popstate 이벤트를 활용하여 뒤로가기 클릭 이벤트 체크 후 팝업창이 열려있으면 팝업창을 닫고 이벤트 종료 - 페이지 뒤로가기 이벤트 막기</p>
                                <p>window 이벤트 내부에선 이벤트가 등록된 시점에서의 state만 가져올수 있는 이슈로 바뀐 state 값을 가져오지 못하여 전역변수 활용하여 해결 (그외에 useRef 사용가능)</p>
                            </div>
                        </div>

                        <div className="mt-[12px] flex items-center gap-[4px]">
                            <p className="heading-xs text-gray060">▶ next13버전에서 14버전으로 업그레이드 후 초기 진입속도 1.5초 가량 단축</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-[8px] flex gap-[4px] ">
                    ...
                    <div className="flex flex-col gap-[2px]">
                        ...
                        <div className="mt-[12px] flex items-center gap-[4px]">
                            <p className="heading-xs text-gray060">▶ 개발 전, 이미 작업중인 프로젝트 및 마이그레이션 업무 특성 생각하여 커뮤니케이션에 비중을 둠</p>
                        </div>
                        <div className="mt-[12px] flex items-center gap-[4px]">
                        ...
                        <div className="mt-[12px] flex flex-col gap-[4px]">
                            <div className="flex items-center gap-[4px]">
                                <p className="heading-xs text-gray060">▶ 카카오톡 내 브라우저, 삼성 브라우저등 디버깅 설정이 어려운 환경에서 이슈 발생</p>
                            </div>
                            <p className="w-fit p-[8px] body-xs text-white bg-blue070/50 rounded-[8px] shadow-shadow16_15">sentry 도입 후 이슈 발생한 부분에 error throw 후 모니터링 하여 이슈체크 후 해결</p>
                        </div>
                ...
                <div className="mt-[8px] flex gap-[4px] ">
                    ...
                    <div className="flex flex-col gap-[2px]">
                        ...
                        <div className="mt-[12px] flex flex-col gap-[4px]">
                            <div className="flex items-center gap-[4px]">
                                <p className="heading-xs text-gray060">▶ 간판제작업체 전용페이지에서 모든페이지에 안읽은 채팅 갯수 및 받은 견적을 보여줘야하는 ui</p>
                            </div>
                            <div className="w-fit p-[8px] flex flex-col body-xs text-white bg-blue070/50 rounded-[8px] shadow-shadow16_15">
                                <p>redux 도입 후 간판제작업체 유저 데이터를 redux로 관리하도록 개발</p>
                            </div>
                        </div>중
                        ...
                        <div className="mt-[12px] flex flex-col gap-[4px]">
                            <div className="flex items-center gap-[4px]">
                                <p className="heading-xs text-gray060">▶ 간판제작을 위한 견적서 작성 프로세스를 페이지가 아닌 view change로 기획되어 브라우저 뒤로가기 클릭시 페이지 이동이 아닌 이전 view를 보여줘야하는 이슈</p>
                            </div>
                            <p className="w-fit p-[8px] body-xs text-white bg-blue070/50 rounded-[8px] shadow-shadow16_15">위에서 작성한 첫번째 프로젝트 모달창 이슈 해결방법과 동일하게 해결</p>
                        </div>

                        <div className="mt-[12px] flex flex-col gap-[4px]">
                            <div className="flex items-center gap-[4px]">
                                <p className="heading-xs text-gray060">▶ 당시 네이티브 앱이 웹을 보여주는 웹뷰만 있던 형식으로 웹에 심어져 있는 이벤트 트래킹 앱 웹뷰에서 발생한건지, 웹에서 발생한건지 분기 처리 이슈 발생</p>
                            </div>
                        ...
                        <div className="mt-[12px] flex flex-col gap-[4px]">
                            <div className="flex items-center gap-[4px]">
                                <p className="heading-xs text-gray060">▶ 웹 사이트 seo가 제대로 작동하지 않던 이슈 발생</p>
                            </div>
                            <div className="w-fit p-[8px] flex flex-col body-xs text-white bg-blue070/50 rounded-[8px] shadow-shadow16_15">
                                <p>초기엔 react helmet 도입하여 테스트 및 꾸준히 수정</p>
                                <p>아예 적용조차 되지않는 이슈로 helmet의 한계점 파악 후, 동적 meta정보가 담긴 문서를 반환하는 api를 (node.js)제작하여 aws lambda 사용하여 seo 봇 접근시 호출되게 배포환경 수정 및 seo 적용 확인</p>
                            </div>
                        </div>
                ...
                {/*1.웹 메인 서비스*/}
                {/*2.간판 제작업체 전용 페이지*/}
                {/*3.서비스 관리용 페이지(영업 및 기획)*/}
                {/*총 3개가 같은 도메인으로 일반 사용자는 2, 3번 페이지에 접근하면 안되는 이슈*/}
                {/*◦ Route component를 커스텀하여 login 안되어있을경우, login후 user 타입이 고객일 경우를 체크하여 Redirect return 또는 그 반대일 경우 해당 page의 component를 return 하게끔 제작*/}
