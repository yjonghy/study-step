...
export default function HtmlCss() {
    ...
    const scriptTagRef = useRef<HTMLDivElement>(null)
    ...
    return (
        <section className={parentStyle}>
            ...
            <div className="fixed left-[10px] bottom-[10px] w-fit flex flex-col gap-[10px] flex-wrap">
                <div
                    ...
                    <p className="text-white body-sm">브라우저 렌더링</p>
                </div>
                <div
                    onClick={() => { if (scriptTagRef.current) scollMove(scriptTagRef.current.getBoundingClientRect().top - 20) }}
                    className={`${PrimaryButton} ${SmallBtnStyle} px-[8px] rounded-[8px]`}>
                    <p className="text-white body-sm">script 태그 옵션</p>
                </div>
            </div>

            <article className="w-full h-full flex gap-[1px]">
